...
public class ClientConnections {
  ...
  public ClientConnections() {
    FXMLLoader fxmlLoader =
        new FXMLLoader(
            getClass().getResource(CONNECTIONS_FXML),
  ...
  private final class PlayerListCell extends ListCell<Player> {
    ...
    protected void updateItem(Player player, boolean empty) {
      ...
      if (empty || player == null || player.getName() == null) {
        ...
        if ((MapTool.isHostingServer() || MapTool.getPlayer().isGM())
            && !MapTool.getPlayer().equals(player)) {
  ...
  private void kickConnectedPlayer(Player selectedPlayer, boolean banPlayer) {
    if (MapTool.isPlayerConnected(selectedPlayer.getName())) {
      ...
      String msg;
      if (banPlayer) msg = I18N.getText("msg.confirm.banPlayer", selectedPlayer.getName());
      else msg = I18N.getText("msg.confirm.kickPlayer", selectedPlayer.getName());
        ...
        if (banPlayer) {
          ...
          if (MapTool.isPlayerConnected(selectedPlayer.getName()))
            ...
            MapTool.showInformation(
                I18N.getText("msg.info.playerBooted", selectedPlayer.getName()));
        }
      }
