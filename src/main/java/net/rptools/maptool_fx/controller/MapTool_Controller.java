...
public class MapTool_Controller {
  ...
  private static final String CONNECTIONS_ICON =
      "/net/rptools/maptool/client/image/dock/connections.png";
  private static final String MAP_EXPLORER_ICON =
      "/net/rptools/maptool/client/image/dock/mapExplorer.png";
  private static final String DRAW_EXPLORER_ICON =
      "/net/rptools/maptool/client/image/dock/drawExplorer.png";
  private static final String INITIATIVE_ICON =
      "/net/rptools/maptool/client/image/dock/intiative.png";
  private static final String RESOURCE_LIBRARY_ICON =
      "/net/rptools/maptool/client/image/dock/resourceLibrary.png";
  ...
  private static final String GLOBAL_ICON =
      "/net/rptools/maptool/client/image/dock/localMacros.png";
  private static final String CAMPAIGN_ICON =
      "/net/rptools/maptool/client/image/dock/campaignMacros.png";
  private static final String SELECTED_ICON =
      "/net/rptools/maptool/client/image/dock/selectedMacros.png";
  private static final String IMPERSONATED_ICON =
      "/net/rptools/maptool/client/image/dock/impersonatedMacros.png";

  private static final Image DOCK_IMAGE =
      new Image("/net/rptools/maptool/client/image/dock/docknode.png");
  ...
  void initialize() {
    assert rootPane != null
        : "fx:id=\"rootContainer\" was not injected: check your FXML file 'MapTool.fxml'.";
    assert menuBar_Controller != null
        : "fx:id=\"menuBar_Controller\" was not injected: check your FXML file 'MapTool.fxml'.";
  ...
  public void setDefaultPanes(MapToolFrame clientFrame, DockPane dockPane) {
    ...
    dockNodes.put(
        "connectionsWindowMenuItem",
        addDockNode(
            clientConnections.getRootNode(), clientConnections.WINDOW_NAME, CONNECTIONS_ICON));
    dockNodes.put(
        "mapExplorerWindowMenuItem",
        addSwingNode(clientFrame.getTokenTreePanel(), MAP_EXPLORER_ICON));
    dockNodes.put(
        "drawExplorerWindowMenuItem",
        addSwingNode(clientFrame.getDrawablesTreePanel(), DRAW_EXPLORER_ICON));
    dockNodes.put(
        "intitiativeWindowMenuItem",
        addSwingNode(clientFrame.getInitiativePanel(), INITIATIVE_ICON));
    dockNodes.put(
        "resourceLibraryWindowMenuItem",
        addSwingNode(clientFrame.getAssetPanel(), RESOURCE_LIBRARY_ICON));
    dockNodes.put("chatWindowMenuItem", addSwingNode(clientFrame.getCommandPanel(), CHAT_ICON));
    dockNodes.put(
        "tablesWindowMenuItem", addSwingNode(clientFrame.getLookupTablePanel(), TABLES_ICON));
    dockNodes.put("globalWindowMenuItem", addSwingNode(clientFrame.getGlobalPanel(), GLOBAL_ICON));
    dockNodes.put(
        "campaignWindowMenuItem", addSwingNode(clientFrame.getCampaignPanel(), CAMPAIGN_ICON));
    dockNodes.put(
        "selectedWindowMenuItem", addSwingNode(clientFrame.getSelectionPanel(), SELECTED_ICON));
    dockNodes.put(
        "impersonatedWindowMenuItem",
        addSwingNode(clientFrame.getImpersonatePanel(), IMPERSONATED_ICON));
  ...
  private DockNode addDockNode(Node node, String dockName, String graphicURI) {
    ...
    dockNode
        .closedProperty()
        .addListener(
            (arg, oldVal, newVal) -> ((CheckMenuItem) dockNode.getUserData()).setSelected(!newVal));

    return dockNode;
  }

  private ImageView setGraphicIcon(String graphicURI) {
    if (graphicURI.isEmpty()) return new ImageView(DOCK_IMAGE);
  ...
  public void showWindow(CheckMenuItem checkMenuItem) {
    ...
    if (checkMenuItem.isSelected()) {
      ...
      if (dockPosition == null) dockPosition = DockPos.LEFT;

      dockNode.setPrefSize(100, 100);

      if (dockSibiling != null) dockNode.dock(dockPane, dockPosition, dockSibiling);
      else dockNode.dock(dockPane, dockPosition);
